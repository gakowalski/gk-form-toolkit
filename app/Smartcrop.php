...
class Smartcrop {
	...
	public static function asset($path, $width, $height, $relative_path_to_public = '.') {
		$prefix = realpath($relative_path_to_public) . DIRECTORY_SEPARATOR;
		$new_path = self::thumbnail("$prefix$path", $width, $height);
		return $new_path ? strtr($new_path, [ $prefix => '']) : asset($path);
	}
	...
	public static function thumbnail($path, $width, $height) {
		...
		if (file_exists($thumbnail_path) === false) {
			$s = new Smartcrop($path, compact('width', 'height'));
			if ($s) {
				$res = $s->analyse();
				$s->crop($res['topCrop']['x'], $res['topCrop']['y'], $res['topCrop']['width'], $res['topCrop']['height']);
				$s->output($thumbnail_path);
			}
		}
